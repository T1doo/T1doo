...
## v1.1.0-m9 — 2026-07-17（M9 状态感知 v2 · hooks 退役，预览）

- **hooks 机制整体退役**：不再向 `~/.claude/settings.json` 注册任何 hook，不再为每个事件 spawn `cmd`+`curl`，本机 HTTP 回调服务（HookServer）删除；设置页「实时状态感知（hooks）」区块与首启引导的授权步一并移除（引导第三步改为纯说明页，无授权动作）
- **升级自动清理**：首次启动检测并**精确摘除** v1.0 留在 `settings.json` 里的注册条目——按命令串中的 `/t1doo-hook` 标记识别，你自己的 hook 与其余键分毫不动；清理前自动备份（`.bak-t1doo`），完成后在指挥台一次性告知。配置解析失败时只记日志、绝不覆盖
- **状态感知改为读取 Claude Code 自己写的会话记录**：零配置、零授权、无开关——内置终端与你在外部终端手开的会话一视同仁。工作中 / 等待确认 / 空闲照常显示，等待时照常通知
- **「等待确认」判定分层，宁可漏报不打扰**（依据本机 4,688 次真实工具调用的耗时分布）：
  - `AskUserQuestion` / `ExitPlanMode` 悬挂 → **确定**判定，立即提示（实测 0.34s），角标填实
  - 会弹确认框的操作悬挂超 2 秒 → **推断**判定（实测 2.4s，守住 3 秒承诺），角标**空心** + tooltip 说明「也可能只是工具执行较慢」
  - 子代理 / 联网搜索等本就漫长、又不需要确认的操作 → 永不提示；`bypassPermissions`/`auto`/`plan`/`dontAsk` 模式下不弹框的操作一律抑制
- 冷启动追平历史会话不再误报状态、不再误发通知（v1.0 的降级路径会把所有历史会话短暂刷成「工作中」）
- `~/.claude/settings.json` 的读写统一到同一底座（读失败即中止绝不覆盖、写前备份、临时文件原子替换）——全局模型切换与本次退役清理共用
- 修复：全局切换与 hooks 各自实现的配置读写行为不一致（前者有原子写、后者没有）
- 测试：状态机单测 27 项 + 退役清理 13 项 + 解析器信号 4 项（168→205）；新增 `e2e-status.cjs` 十步全链路（直接按真实格式写 JSONL，零额度）；M1-M8 八脚本 E2E 回归全绿
