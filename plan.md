...
| 新 assistant 行含悬挂 `tool_use` | → `working`；够格的工具静默超阈值后 → `waiting` + 系统通知（分层判定见下） |
| tool_result（user 行）到达 | → `working`（确认已给出 / 工具完成） |
| **末行为 assistant** 且无悬挂 tool_use，落定 1.5s | → `idle`（回合结束） |
| **末行为 user** 且长时间无写入（5min） | → `idle`（视作绑定进程已死） |

**waiting 分层判定（M9 实测定档，取代原「一刀切 2s 阈值」）**——依据：本机 80 个会话文件、4,688 次 `tool_use → tool_result` 配对的执行耗时分布（§14.2）：

| 层 | 判定 | 依据 |
|----|------|------|
| ① **确定层**：`AskUserQuestion` / `ExitPlanMode` 悬挂 | 立即 `waiting`，`certain=true`，**无阈值、不受 permissionMode 影响** | 这两个工具的语义**就是**等用户回答/批准。实测 n=21、P(耗时>2s)=100%、中位 51s |
| ② **启发层**：该工具在当前 `permissionMode` 下**会弹确认框**且悬挂静默 ≥2s | `waiting`，`certain=false`（空心角标） | 文件类工具执行近乎瞬时（Edit 中位 0.03s、P(>2s)=0.7%；Read 4.2%），故 2s 悬挂几乎必然是在等人 |
| ③ **排除层**：`Agent`/`Task`/`WebSearch`/`WebFetch`/`Skill`/`Workflow`/`Task*`/`ToolSearch` | **永不**据此推 waiting | 既不弹确认、执行又本就漫长（Agent 中位 69s、最长 998s）——悬挂无信息量 |

- **为何不能一刀切**：全量 P(执行>2s)=23.8%，但**全部集中在命令/长任务类**（Bash 71.9%、PowerShell 94.1%、Agent 69.5%、WebSearch 95.9%），文件类仅 0.7–6.1%。原方案的统一阈值会把这 24% 变成误报通知——对「因嫌 hooks 吵而退役 hooks」的用户是反效果。取舍定为**宁可漏报不误报**（用户裁决，2026-07-17）。
- **permissionMode 抑制**（启发层）：`bypassPermissions`/`dontAsk`/`auto`/`plan` → 全抑制；`acceptEdits` → 抑制 Edit/Write/NotebookEdit，Bash 等仍推；`default`/未知 → 生效。
- **permissionMode 采集口径**（实测修正）：不止「JSONL 行自带」——① 真实用户提示行的顶层 `permissionMode` 字段（覆盖率 90.2%，且**首次得知恒在第 1 个提示**，80 文件中 77 个如此）；② **专门的 `{"type":"permission-mode","permissionMode":…,"sessionId":…}` 行**（会话中途 shift+tab 改模式时写入，是变更的权威来源，v1.0 解析器把它丢进了 skipped 桶）。两者按行序 sticky 覆盖。侧链行不带该字段——本就不参与主状态。
- **末行角色必须采集**：回合收尾的 assistant 纯文本行不产生任何其他信号，唯有靠「末行角色」才能把「回合结束」（末行 assistant → idle）与「CC 正在调 API 思考」（末行 user → 恒 working）区分开；缺了它 idle 只能等 5min 兜底。
- **落定延迟 1.5s 的由来**：CC 把**一条** assistant 消息按内容块**拆成多行**写（同一 `message.id` 的 text 行与 tool_use 行相邻）——无落定延迟会在两行之间闪出假 idle。
- **如实展示局限**：无法区分「等确认」与「工具执行慢」，启发层 waiting 为**推断值**——UI 空心角标 + tooltip 标注（`statusCertain=false`）；确定层才填实。实测通知延迟：启发层 2.4s（2s 阈值 + 300ms 防抖）、确定层 0.34s，U4 的 3 秒承诺守住。
- **判活门槛**：仅最后一行在 5 分钟内的会话进状态机——冷启动追平数百个历史文件时零状态、零通知（E2E 以「含悬挂 Edit 的 7 天前会话」把关）。
- **覆盖范围优于 hooks 默认态**：内置终端与外部手开会话一视同仁（同一数据源），无需任何注册动作。

**已核实不可用的「更好信号」**（M9 调研，均因版本漂移剧烈而弃用，属 R1 风险的实例）：`queue-operation`（enqueue/dequeue，理论上是精确的 working 起点）在 2.1.211 上 12 个提示只发 3 次 enqueue，而 2.1.209 的某会话 9 个提示发了 34 次；`system.subtype=turn_duration`（理论上是确定的回合结束信号）377 个回合仅出现 20 次。故状态机**只依赖 user/assistant 行这一稳定核心格式**——原方案的保守取向经此验证是对的。

#### 7.9.3 OTEL spike（0.5 天，采信须实测）

内置终端 spawn 时注入 `CLAUDE_CODE_ENABLE_TELEMETRY=1` + `OTEL_TRACES_EXPORTER=otlp` + `OTEL_EXPORTER_OTLP_ENDPOINT=http://127.0.0.1:<port>` + `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1`，T1doo 起本地最小 OTLP/HTTP 接收端，实测交互式会话是否发出 `claude_code.tool.blocked_on_user` 跨度及其延迟。**成立** → v1.2 为内置终端接入确定性 waiting 信号（外部会话仍走 JSONL 推断）；**不成立** → JSONL 推断即最终方案。结论记 §14.2。

> 🕒 **状态：待做**（2026-07-17 用户裁决——M9 主线交付后单独跑，不阻塞交付）。必须消耗真实额度：要验证的正是「**交互式** CLI 会不会发」，假 claude 测不了，且 `blocked_on_user` 需要真实弹出权限确认框。
>
> ⚠️ M9 实测已提高其价值：JSONL 里**不存在任何「权限请求」行类型**（全量行类型已枚举核实，§14.2），因此 waiting 在 JSONL 侧**注定无法确定化**——分层判定中的启发层已是该数据源的上限。OTEL 若成立，是内置终端拿到确定 waiting 的**唯一**已知通道。

#### 7.9.4 退役与迁移

- **删除**：HookServer（127.0.0.1 HTTP 服务）、hooks 注册侧（`buildHookCommand`/`mergeHooks`/`HOOK_EVENTS`）、设置页 HooksSection、首启引导 hooks 步（改为状态感知说明页，无授权动作）、`hooks:get-state`/`hooks:set-enabled` 两条 IPC 与 `HooksState` 类型。**保留移除侧**（`removeHooks`/`hasOurHooks`）→ 迁入 `services/claude/hooks-retire.ts`，专供升级清理。
- **公共底座抽取**（实装偏差修正）：原计划「深合并/精确移除工具函数移交 §7.7.5 全局切换复用」——**前提不成立**：全局切换并未复用 hooks 模块，而是自持了一份 `readSettings/writeSettings`，且两者范式不通用（hooks 认「命令串含 `/t1doo-hook`」内容标记，env 认「managedKeys 记账名单」），各自 own settings.json 的不同子树。真正重复的是**文件 IO 那层**（两份 read/write，且 hooks 版无备份、非原子写）→ 抽出 `services/claude/settings-io.ts`（读失败即抛绝不覆盖 + 备份 + temp/rename 原子写），全局切换与退役清理共用，行为就此统一到更稳的那份。
- **升级清理**：v1.1 首次启动检测 `~/.claude/settings.json` 中带 `/t1doo-hook` 标记的条目 → 自动精确移除（沿用既有移除逻辑与「深度相等」测试口径），其余键分毫不动；解析失败只记日志不打断启动。清理完成后 UI 一次性告知——标记**落盘**（`status` store）而非内存：T1doo 可隐藏启动到托盘，内存标记会让用户根本看不到。
- **绑定路径补偿**：SessionStart 权威校正随 hooks 退役取消，但 `adoptSession`（按 cwd 关联未绑定终端）**未废弃**——改由状态机在 JSONL **首见**某活跃会话时调用，「shell 终端里手敲 claude」这一路的绑定得以保留（E2E 已把关）。
- **不受影响**：任务队列 `--include-hook-events`（stream-json 内联事件，与 HookServer 无关，§7.5.2）；`--session-id` 主绑定路径（§7.2.3，R8 已实测）。
- **原则修订**：`~/.claude` 唯一写入例外由「hooks 注册」变更为「模型中心全局切换 env 键」（§1.4 / §11 已同步）。M9 后 T1doo 对 `~/.claude` 的写入只剩两处：全局切换 env 键、以及这一次性的退役清理。
...
| **M8 用量中心（F9，v1.1）** ✅ 2026-07-07 代码交付 | 第 15-16 周（实际与 M7 同日） | 「用量」一级板块；usage_log 独立采集管道（subagents/wf_* 全覆盖、message.id 去重、cache 四维、api/cli 面板来源并入、任务不双采）；六档时间范围（含自定义日期区间）；Recharts 趋势/分布图 + Hero 指标卡 + 缓存命中率；价目表可编辑 + 名义成本开关；Dashboard 卡片精简接跳转 | ① ✅ 首扫全覆盖实测 7.0-8.3s（1,770 文件 / 12k 行，预算 30s），期间 UI 最大响应延迟 165-216ms 无卡顿（perf-audit ④ + E2E `scripts/e2e-usage.cjs` ①）；② ✅ 去重与「任一 token>0」口径 fixtures 单测（7 项）+ E2E 同 id 快照去重断言；**ccusage 对拍总误差 0.389% < 1%**（`scripts/usage-vs-ccusage.cjs`，30 天 9.7 亿 token；仅顶层口径差 24.4%——反证 subagents 覆盖必需）；③ ✅ 聚合正确性单测 19 项（本地时区 23:59:59.999 切日、6/7 月跨月、48h/92 天分桶边界、补零连续、null-ts 排除、三维筛选）；④ ✅ §7.8.5 四项全达标并入 perf-audit：首扫 7.0s<30s / 增量 209ms<300ms（防抖压至 150+50ms）/ 聚合 6.6-10.9ms<100ms 含 IPC / usage 表 2.8MB<30MB（§10.3 老三项同跑：0.78s/333MB/0.04%）；⑤ ✅ fixtures 注入 E2E：Recharts 柱/面积双模式渲染 + 暗/亮双主题截图（E2E ②）；⑥ ✅ 定价归一匹配单测（前缀/日期后缀/`.`变体/最长前缀/不误配）+ Decimal 定点成本零浮点误差；成本开关默认关、开启恒带「估算」+ 内置价 $3.095 精确断言 + 价目编辑/重置联动（E2E ④⑤）；⑦ ✅ M1-M7 六脚本 E2E 回归全绿；单测 109→168 项。**真实第三方网关面板计费留手动点验**（M2/M7 同口径） |
| **M9 状态感知 v2 · hooks 退役（F2，v1.1）** 🔨 2026-07-17 代码交付 | 第 17 周 | JSONL 事件驱动状态机成为默认且唯一路径（复用 F1 增量管道，新增行级出口；**waiting 改为按「工具类 × permissionMode」分层判定**，§7.9.2）；hooks 全面退役：HookServer / 注册侧 / 设置页 HooksSection / 首启引导 hooks 步 / 两条 IPC 删除，升级时自动清理既有注册；settings.json 文件 IO 抽公共底座；OTEL spike（§7.9.3）经用户裁决推迟至主线交付后单独跑 | ① ✅ 状态机单测 27 项（全流转 + 阈值边界 2s±1ms + 确定/启发/排除三层 + permissionMode 五模式抑制 + 判活 + 生命周期）+ 解析器信号提取 4 项（真实形状 fixtures：`permission-mode` 行 / 侧链排除 / tool_result 区分末行角色）；② ✅ E2E `scripts/e2e-status.cjs` 十步全通（**直接按真实行格式追加写 JSONL，无需假 claude，零额度**）：悬挂 Edit → waiting **2,435ms ≤3s**（U4 守住）、AskUserQuestion → 确定 waiting **342ms**、tool_result → 回 working、回合收尾 → idle；③ ✅ 升级清理：预置带 `/t1doo-hook` 注册的 settings.json → 启动后精确移除、其余键深度相等、备份为清理前原样（E2E ① + 单测 13 项，含 JSON 坏/顶层数组时抛错不覆盖）；④ 🕒 OTEL spike 待做（不阻塞交付，§7.9.3）；⑤ ✅ M1-M8 八脚本 E2E 回归全绿（e2e-terminal 的 hooks 三步改写为「settings.json 分毫未动」把关）；单测 168→205 项。另：③ 冷启动追平含悬挂 Edit 的 7 天前会话 → 零状态零通知；⑩ Agent 悬挂 4s + bypassPermissions → 不误报 |
...
| R10 | 第三方/自定义后端与 Claude Code 兼容性参差（工具调用、`stream-json` 格式、`--session-id` 支持差异） | 中 | 中 | 后端档案标注能力；订阅态为默认与保底；异常时明确提示"该后端不支持 X"；解析层对缺字段容错。**2026-07-07 实证（开发机全局切 DeepSeek）**：主对话可用，但 Claude Code 内部以 `[1m]` 缓存后缀模型名（如 `deepseek-v4-pro[1m]`）调用后台小模型（含权限分类器）被 DeepSeek 兼容端点拒绝——正在工作的会话被打瘫，且**运行中进程不热感知 env 键移除，须重启会话恢复**。缓解：试用第三方首选按终端覆盖（§7.7.5），全局切换避开工作会话进行时 |
...
| 2026-07-07 | **M8 交付 + 实装结论五则**：① 采集架构偏差（对 §7.8.2 第 1 条的简化）——顶层主会话**不复用 F1 顺带产出**，改为用量扫描器统一扫全部 JSONL（顶层+subagents/wf_*）：F1 游标已消费的存量内容不会再过 F1 管道，"顺带产出"路径必然要配一次性回填，两条写入路互相纠缠；统一扫描器只多付一次顺序读，7.2-8.3s 完成全量首扫（预算 30s），换来单管道/单游标表/REPLACE 幂等，F1 一行未动；② 真实目录布局实测：wf_* 在 `<sessionId>/subagents/workflows/wf_*/` 之下（计划原文 `wf_*/**.jsonl` 位置有误），`journal.jsonl` 与 `*.meta.json` 同层混布——扫描按 `subagents/**` 递归 + 行级 `type=="assistant"` 过滤天然排掉非转录文件；③ 量测方法坑两枚：聚合查询在 F1 初始同步期间量测会把主线程写库的 IPC 排队算进查询头上（233ms→平息后 3.9ms，差 60 倍）；内存量测必须在查询爆发前（15 连发查询瞬时页 Windows 懒回收，虚增 ~100MB：434 vs 331）——perf-audit 顺序定为"平息→内存→查询→CPU"；④ vitest 跑 better-sqlite3 的 Electron ABI 冲突用**别名副本**解法：`better-sqlite3-node@npm:better-sqlite3@12.10.1` 装 Node ABI 版 + vitest resolve.alias 重定向——版本刻意与主依赖错开，防 pnpm store 去重共享物理目录被 install-app-deps 重建波及（DAO/聚合 SQL 得以在内存库上真跑）；⑤ 面板 cli 回合主键实现为 `cli:<sessionId>:<convMessageRowId>`（行 id 单调且稳定，充当 turn 序号）；顺带修复 settings sanitize 白名单漏 `notifyTaskDone` 致该开关永不落盘的 v1.0 遗留 bug。增量管道防抖压到 150+50ms 满足 <300ms 预算（实测 209ms） | M8 实装结论（§7.8/§9 M8 行） |
| 2026-07-17 | **M9 调研实证：waiting 无法确定化，且「一刀切 2s」会有 24% 误报 → 改分层判定（用户裁决）**：全量枚举本机 JSONL 行类型（80 文件）证实**不存在任何「权限请求」行**——waiting 在 JSONL 侧注定是推断值，原方案判断正确。但按 4,688 次 `tool_use→tool_result` 配对实测执行耗时：全量 P(>2s)=23.8%，**全部集中在命令/长任务类**（Bash 71.9% / PowerShell 94.1% / Agent 69.5%（中位 69s、最长 998s）/ WebSearch 95.9%），而文件类近乎瞬时（Edit 中位 0.03s、P(>2s)=0.7%；Read 4.2%）。→ 统一阈值＝把 24% 的慢工具误报成「等你确认」，对「因嫌 hooks 吵而退役 hooks」的用户是反效果。**定案分层**：① 确定层 `AskUserQuestion`/`ExitPlanMode` 悬挂即 waiting（语义就是等人，实测 n=21/P(>2s)=100%/中位 51s，无阈值、零误报——原方案未预见）；② 启发层＝该工具在当前 permissionMode 下会弹框且静默≥2s；③ 排除层 Agent/WebSearch 等永不推。取舍：**宁可漏报不误报** | 用户裁决（§7.9.2 已重写）；实测数据见本行 |
| 2026-07-17 | **M9 调研：三处对 §7.9 原文的实测修正** | ① `permissionMode` 采集口径：不止「行自带」——真实用户提示行覆盖率 90.2% 且**首次得知恒在第 1 个提示**（77/80 文件），另有专门的 `type:"permission-mode"` 行承载中途改模式（v1.0 解析器丢进 skipped 桶）→ 两者 sticky 覆盖，未知时按 default 处理（未知窗口极短，功能不哑火）；实测值域 auto/bypassPermissions/default/plan（+ 共享类型已有的 dontAsk/acceptEdits）。② **「更好的信号」是陷阱**：`queue-operation`（精确 working 起点）与 `system.subtype=turn_duration`（确定回合结束）覆盖率随 CC 版本剧烈漂移——2.1.211 上 12 提示仅 3 次 enqueue、2.1.209 某会话 9 提示发 34 次；turn_duration 377 回合仅 20 次 → 全部弃用，状态机只依赖 user/assistant 行这一稳定核心格式（R1 风险实例，原方案保守取向经此验证正确）。③ CC 把**一条 assistant 消息按内容块拆多行写**（同一 message.id 的 text 行与 tool_use 行相邻）→ 「无悬挂即回合结束」会在两行间闪假 idle，须加 1.5s 落定；且回合收尾的纯文本行**不产生任何信号**，必须采集「末行角色」才能区分「回合结束」与「CC 正在调 API 思考」（后者末行是 user，恒 working，绝不能凭静默判 idle）——此项由单测在实装中当场逼出 |
| 2026-07-17 | **M9 交付 + 实装偏差三则**：① §7.9.4 原文「深合并/精确移除工具函数移交全局切换复用」**前提不成立**——全局切换从未复用 hooks 模块而是自持一份 settings.json 读写，且两者范式不通用（内容标记 vs managedKeys 记账），各 own 不同子树；真正重复的是文件 IO 层（hooks 版还缺备份与原子写）→ 抽 `services/claude/settings-io.ts` 公共底座，两边共用更稳的那份；移除侧 `removeHooks/hasOurHooks` 迁 `hooks-retire.ts` 专供升级清理，注册侧全删。② **F1 的行级信息止步于 DAO**（`emitSessionsUpdated` 只传 sessionIds）→ 新增 `emitStatusSignals` 行级出口，解析器产出 `StatusSignals`（permissionMode/悬挂开闭/末行角色/判活时间戳，侧链行不参与）；冷启动追平历史文件同样会发，由状态机按行内时间戳**判活**（5min 窗）自行丢弃——这是「冷启动不误报」的关键，v1.0 的 touchFromSync 降级路径恰恰会把全部历史会话刷成 working。③ hooks 退役会切断 `adoptSession`（shell 里手敲 claude 的唯一绑定路径）→ 改由状态机在 JSONL 首见活跃会话时调用，绑定能力未丢。另：退役告知标记**落盘**而非内存（T1doo 可隐藏启动到托盘，内存标记用户根本看不到）；状态机用截止点 setTimeout 精确唤醒而非常驻轮询（§10.3 空闲 CPU 预算 + EcoQoS 下定时器本就退化到秒级）| M9 实装结论（§7.9/§9 M9 行） |
...
*Plan.md v1.7 · 起草于 2026-07-03，同日据 §14.1 Q1–Q7 裁决升级 v1.0；2026-07-04 经本机无头实测 + 官方文档核对补强为 v1.1；同日 M0-M3 全部交付后，F4 文件中枢裁撤出 v1 升级 v1.2（Claude Fable 5）；2026-07-05 M5 AI 能力交付 + F4 彻底废弃（不再实现）升级 v1.3；2026-07-07 M6 打磨发布代码交付（i18n/首启引导/自动更新/发布流水线/§10.3 审计全达标）升级 v1.4；同日 v1.1 周期规划立项（F8 模型中心 + F9 用量中心 → M7/M8，参照 cc-switch 源码调研，§7.7/§7.8）升级 v1.5；同日 Q8 裁决通过（全局切换转正）+ hooks 退役决定（§7.9，新增 M9，v1.1 共 5 周）升级 v1.6；同日 M7 模型中心交付（PR #8）、M8 用量中心交付（PR #9）。2026-07-17 M9 状态感知 v2 · hooks 退役交付（§7.9 按实测重写：waiting 改分层判定、permissionMode 采集口径修正、settings-io 公共底座）升级 v1.7。剩余：OTEL spike（§7.9.3，需真实额度）→ v1.1.0 tag + Release；干净虚拟机全流程手动验证（v1.0 验收③）。*
