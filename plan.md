...
> **文档状态**：v1.0（§14.1 的 Q1–Q7 已拍板；据此升级，可启动 M0）
> **创建日期**：2026-07-03 · **升级 v1.0**：2026-07-03
...
| P4 | 常用文件散落各处，"上周 Claude 改过的那个文件在哪"无从查起 | 文件中枢：目录订阅索引 + 最近/收藏/标签，并与会话记录联动 |
...
| U6 | 我能订阅几个常用目录（项目区、下载区、文档区），按文件名秒搜，并看到"最近打开/最近被会话修改"的文件流 | F4 |
...
| **F4 文件中枢（CC 工作流入口）** | **会话-文件联动:文件被哪些会话动过 / 最近被会话改的文件流** | Must | M4（数据 M1 采集） |
| | 目录订阅索引 + 实时增量 | Must | M4 |
| | 文件名秒搜、类型/时间筛选、最近/收藏/标签、右键操作 | Must | M4 |
| | Everything 集成（"够用"层,检测到则可选全盘搜索） | Should | M4 |
...
'files:search'         (q: string, opts) => FileHit[]
...
'sessions:files'       (path: string) => SessionRef[]        // F4 联动：动过此文件的会话
...
| 路径 | 内容 | T1doo 用法 |
|------|------|-----------|
| `projects/<slug>/<sessionId>.jsonl` | **会话全文记录**（追加写、一行一事件） | F1 核心数据源，只读 + 增量解析 |
...
**slug 规则（实测）**：项目绝对路径中 `:` `\` `.` 空格等字符替换为 `-`，如 `E:\T1doo` → `E--T1doo`。注意 slug 有歧义可能（不同路径映射同名），因此**以 JSONL 行内的 `cwd` 字段为项目路径的权威来源**，slug 仅用于目录定位。

**JSONL 行类型（实测 + 已知）**：`user`、`assistant`、`system`、`summary`、`queue-operation`、`file-history-snapshot`、`progress` 等。解析器必须**按类型白名单处理、未知类型静默跳过**（格式是 Claude Code 内部实现，无兼容性承诺——这是本项目第一大风险，见 §12-R1）。关键字段见附录 A。
...
CREATE TABLE sessions (
  ...
  title TEXT,                       -- 取 summary 行或首条用户消息截断
...
1. **首次全量**：扫描 `projects/*/*.jsonl`，Worker 内逐文件流式解析入库，记录 `jsonl_offset = 文件字节长`。
2. **增量**：chokidar 监听该目录。JSONL 是**追加写**——文件变更时从上次 `jsonl_offset` 继续读新行，只解析增量（一次会话交互通常仅几 KB）。**注意半行**：写入可能被读到一半，故**只把 `jsonl_offset` 推进到最后一个完整 `\n`，尾部残行不入库、留待下次补齐**，避免错位读坏整条会话。
...
- **详情回放**：
  - 消息按 `parentUuid` 构建树，主干线性展示；侧链（`isSidechain=true`，子代理轨迹）折叠为可展开的嵌套块。
...
- ⚠️ **M2 前实测**：`claude --session-id <新uuid>` 能否新建会话、`--resume <id>` 是否保持同 id（决定上面两条主路径是否成立，见 R8）。

#### 7.2.4 实时状态感知（hooks 方案）

**方案**：T1doo 主进程启动 `HookServer`（`127.0.0.1` 随机端口，Bearer token 校验，仅回环地址）。经用户在设置页**显式开启**后，向 `~/.claude/settings.json` 注册 hooks（JSON 合并写入，保留用户已有 hooks，可一键移除并还原备份）：

| Hook 事件 | 上报后的状态推断 |
|-----------|----------------|
| `UserPromptSubmit` | → `working`（开始干活） |
| `Notification` | → `waiting`（等待权限确认/用户输入）+ 系统通知 |
| `Stop` | → `idle`（回合结束） |
...
hook stdin 自带 `session_id`、`hook_event_name`、`cwd`、`transcript_path` 等 JSON 字段，直接转发即可。（`curl.exe` 自 Win10 1809 起随系统预装，与目标平台一致；缺失时降级到 PowerShell `Invoke-RestMethod`。）
...
> ⚠️ **待核实**：`ANTHROPIC_BASE_URL` / `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_MODEL` / `ANTHROPIC_SMALL_FAST_MODEL` 等变量名，以 M2 落地时的当前 Claude Code 版本实测为准（附录 A.4 留验证位），不凭记忆写死。
...
| 输入形态 | 路由 | 实现 |
|---------|------|------|
| 普通词 | **项目 → 会话标题 → 运行中终端 → 最近提示词** → 应用 → 文件 混排 | CC 源优先，各源 top-N 按 frecency 归并 |
...
|---|---|---|
| 通道 | 子进程 `claude -p --output-format stream-json --include-partial-messages` | `@anthropic-ai/sdk` `messages.stream()` |
...
| 能力 | 可带工具/MCP（谨慎默认禁用工具，纯问答） | 纯模型对话，参数可控 |
| 适用 | 个人日常快速问答 | 需要指定模型/系统提示词/无 CLI 环境 |

模型选项（API 引擎，价格为 2026-06 官方缓存价，UI 中展示供选择）：

| 模型 | ID | 定位 |
|------|----|------|
| Claude Opus 4.8（默认） | `claude-opus-4-8` | 最强 Opus，$5/$25 每百万 token |
| Claude Sonnet 5 | `claude-sonnet-5` | 性价比，$3/$15（限时 $2/$10） |
| Claude Haiku 4.5 | `claude-haiku-4-5` | 快速便宜，$1/$5 |

实现要点：流式渲染（delta 事件 → `evt:ai:delta`）；新模型（Opus 4.7+/Sonnet 5）**不提供 temperature 等采样参数 UI**（API 已移除，传参会 400）；thinking 采用 `{type:"adaptive"}`；长输出走流式避免超时。cli 引擎与任务队列均接受 `backendProfileId`（§7.2.6），据此注入后端环境。
...
默认权限模式保守（`default`，工具调用将失败即止）；用户可对信任目录选择 `acceptEdits`。`bypassPermissions` 双重确认。后续演进方向：改用 `@anthropic-ai/claude-agent-sdk` 获得结构化控制（M5 期间评估，不承诺）。

### 7.6 F6 · 指挥台 Dashboard

聚合只读视图（数据全部来自既有表，无新采集）：活跃终端/会话状态卡片、今日/本周 token 消耗曲线（按 assistant 消息 usage 聚合）、等待确认的会话置顶提醒、最近文件、运行中任务。作为默认首页。

**成本口径**：token 数始终展示；**折算美元金额仅对"API 引擎 + 已知 Anthropic 定价的模型"显示**。订阅态会话（含 Max）与自定义/第三方后端不套用 Anthropic 单价，只显示 token 数并标注"订阅内 / 自定义后端"，避免误导。
...
> 工期假设：单人开发 + AI 辅助，每周有效投入 ≈ 20h。总计 **14 周（约 3.5 个月）到 v1.0**。每个里程碑结束时产出可运行、可日常使用的版本（打 tag）。
...
| **M1 会话中心** | 第 2-3 周 | JSONL 发现/容错解析/全量+增量同步；SQLite+FTS5；**解析时顺带采集 tool_use 文件路径 → session_files（供 F4 联动）**；会话列表/搜索/详情回放；外部终端恢复；导出 md/json；fixtures 回归测试 | ① 本机全部历史会话完整入库，抽查 10 个会话渲染无错；② 全量冷同步 1GB 数据 < 60s，增量感知 < 1s；③ 搜索 10 万消息 < 200ms；④ 恢复命令成功拉起 `wt` 并续聊；⑤ 解析器对 fixtures 中"未知类型行/损坏行"零崩溃 |
...
| **M5 AI 能力** | 第 11-12 周 | 对话面板（流式/Markdown/高亮/历史落库可搜）；双引擎（cli 默认 + api 可配，Key 走 safeStorage）；启动器 `@` 提问接通；任务队列最小闭环 | ① 两种引擎均可流式对话且断网/无 Key 等异常有明确提示；② Key 在磁盘上不可见明文（验证 DPAPI 密文）；③ 任务：提交 → 后台执行 → 通知 → 可查看输出与对应会话全程走通；④ 对话历史可全文搜索 |
| **M6 打磨发布** | 第 13-14 周 | 性能与内存审计（§10.3 预算达标）；i18n 补全；首启引导；NSIS+portable 打包；electron-updater（GitHub Releases）；README/使用文档 | ① 冷启动到 Dashboard 可交互 < 3s；② 常驻内存（1 终端+托盘）< 350MB；③ 安装→使用→自动更新→卸载全流程在干净虚拟机验证通过；④ v1.0.0 tag + Release 产物 |
...
| 常驻内存（空闲，1 终端） | < 350MB |
| 启动器唤起 | < 100ms |
| 搜索（消息 10 万 / 文件 10 万） | < 200ms / < 100ms |
| 安装包体积 | < 150MB |
| CPU 空闲占用 | < 1%（无监听风暴） |
...
| # | 风险 | 概率 | 影响 | 应对 |
|---|------|:--:|:--:|------|
| R1 | **Claude Code JSONL/目录结构是内部格式，版本升级可能破坏解析**（最大风险） | 高 | 高 | 白名单容错解析（未知即跳过不崩）；fixtures 回归；`cc_version` 落库便于定位漂移；核心功能（终端/启动器/文件）不依赖解析成功 |
...
| R8 | `claude -p` 无头模式行为/参数随版本变化 | 中 | 中 | 引擎抽象隔离；启动前 `claude --version` 探测并留兼容分支；Agent SDK 作为后备通道 |
...
| 待 M2 | 核实后端注入的环境变量名（`ANTHROPIC_BASE_URL` 等）对当前 Claude Code 版本 | 内部机制无兼容承诺，落地前实测（附录 A.4） |
...
> 实测环境：Claude Code **2.1.196**，Windows 11，2026-07-03。内部格式无兼容承诺，升级后以 fixtures 回归为准。

### A.1 `~/.claude` 目录（实测节选）

```
%USERPROFILE%\.claude\
├── projects\<slug>\<sessionId>.jsonl   # 会话全文（核心数据源）
...
// type: "summary" —— 会话标题摘要（含 leafUuid）
// 其他类型（system / file-history-snapshot / progress ...）：白名单外一律跳过
```
...
| 用途 | 命令 |
|------|------|
| 恢复指定会话 | `claude --resume <sessionId>`（配合 `-d <cwd>` 的 wt 使用） |
| 继续最近会话 | `claude --continue` |
| 指定会话 ID 启动 | `claude --session-id <uuid>`（T1doo 绑定用） |
| 无头执行 | `claude -p "<prompt>" --output-format stream-json --include-partial-messages` |
| 模型/权限 | `--model <id>` / `--permission-mode <mode>` / `--dangerously-skip-permissions`（慎用） |
| 版本探测 | `claude --version`（实测输出 `2.1.196 (Claude Code)`） |
| 切换后端（变量名待核实） | 经 env 注入 `ANTHROPIC_BASE_URL` + `ANTHROPIC_AUTH_TOKEN` + `ANTHROPIC_MODEL` 指向兼容后端；M2 落地前对当前版本实测确认（见 §7.2.6） |
...
{
  "hooks": {
    "UserPromptSubmit": [ { "hooks": [ { "type": "command", "command": "<A.4 上报命令>" } ] } ],
...
> 注册实现要求：JSON 深合并（不覆盖用户既有 hooks）→ 写前备份 `settings.json.t1doo.bak` → 卸载/关闭功能时精确移除自己的条目。
...
*Plan.md v1.0 · 起草于 2026-07-03（Claude Fable 5，本机实测），同日据 §14.1 Q1–Q7 裁决升级。下一步：启动 M0 脚手架。*
